...
# pathway genes 
pathway_genes <- list(
  ferroptosis = c("HERC2", "NFE2L2", "TFRC",  "AIFM2", "SLC7A11", "PIR", "MGST1",  "GPX4", "FECH", "HMOX1", "SAT1"),
  redox = c("GSR", "SOD1", "PRDX1", "PRDX3", "CAT", "TXN"),
  niacin = c("SIRT1", "SIRT2", "NAPRT", "NAMPT", "NMNAT1", "PARP1"),
  glutathione = c("GSTP1",  "GCLC",   "GSTO1", "GSS", "GLS", "GLRX", "GLUL",  "GLUD1", "GCLM", "SLC38A1"),
  glutamine = c("GLS", "SLC1A5", "IDH2", "GLS2", "GLUL", "SLC38A1", "GLUD1"),
  anti_apoptotic = c("BCL2", "MCL1", "BCL2L2", "BCL2A1", "BCL2L1", "BAG3", "XIAP", "BIRC5", "BCL10"),
  presis = c("TFRC", "SLC7A11", "GCLC", "GCLM", "GSS", "GSR", "GLS", "BCL2L1")
)
...
# 6. Plot per pathway, preserving gene order from pathway_genes
pathway_plots <- fibro_summary %>%
  ...
  map(~ {
    ...
    # If you want the *first gene in your list to appear at the TOP* of the plot,
    # uncomment the next line:
    # df <- df %>% mutate(Gene = forcats::fct_rev(Gene))
